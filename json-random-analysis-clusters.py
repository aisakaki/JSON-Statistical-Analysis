...
def integration(n, AD_threshold):
    print("BNE is Both not exist")
    global total_dict, both_not_exist
    for key in total_dict:
        both_not_exist[key] = int(n - total_dict[key] - both_not_exist[key]) / n
    ...
    for (key, value) in total_list:
        ...
        if effective_same_judgement(same_rate, both_not_exist[key], AD_threshold , ES_threshold) is False:
            continue
        if key not in dict_node_list and key not in list_node_list:
            if key not in connection_dict:
                if both_not_exist[key] > 0 and same_rate > AD_threshold:
                    print('【'+key + "】 : %.2f  (BNE: %.2f)" % (same_rate, both_not_exist[key]))
                else:
                    print('【' + key + "】 : %.2f" % same_rate)
            else:
                if both_not_exist[key] > 0 and same_rate > AD_threshold:
                    print(connection_dict[key] + '【' + key + "】 : %.2f  (BNE: %.2f)" % (same_rate, both_not_exist[key]))
                else:
                    print(connection_dict[key] + '【' + key + "】 : %.2f" % same_rate)


# AD threshold:绝对不同门限；ES threshold:有效相似门限
def effective_same_judgement(S, BNE, AD_threshold, ES_threshold):
    if S < AD_threshold:
        return True
    ES = BNE / S
    if ES > ES_threshold:
...
# APah 和 BPath为要抽样比较的两个文件夹的路径，N为抽样样本个数，allFeaturePath为过滤特征文件，AD threshold为绝对不同门限
# 用于按照家族分析特征
def run_path(APath, BPath, N, allFeaturePath, AD_threshold):
    global total_dict, dict_node_list, list_node_list, connection_dict, both_not_exist
    total_dict = {}
    dict_node_list = []
    list_node_list = []
    connection_dict = {}
    ...
    print('\n=============SIMILARITY=============')
    integration(N, AD_threshold)


# clusters_file:聚类结果文件路径, class_no1：A的家族序号, cluster_no1：A的簇类序号, class_path：A特征家族的路径, class_no2：B的家族序号, cluster_no2：B的簇类序号, class_path：B特征家族的路径,N：抽样样本个数,allFeaturePath为过滤特征文件, AD threshold为绝对不同门限
# 用于按照聚类之后簇分析特征
def run_clusters(clusters_file, class_no1, cluster_no1, class_path1, class_no2, cluster_no2, class_path2, N, allFeaturePath, AD_threshold):
    global total_dict, dict_node_list, list_node_list, connection_dict,both_not_exist
    total_dict = {}
    dict_node_list = []
    list_node_list = []
...
run_clusters('cl.txt', 0, 1, 'EXAMPLE', 1, 2, 'EXAMPLE', 10, r'S:\资料库\Desktop\聚类exp\allFeature1479.pickle', 0.5)